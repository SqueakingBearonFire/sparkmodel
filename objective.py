...
# 2. 新的 objective
def objective_ml(params, data_size_gb, job_type):
    """
    用训练好的随机森林模型预测执行时间
    ...
    features = np.array([data_size_gb, jt_enc, em, ec, dm, dc, npart]).reshape(1, -1)

    # 将特征转换为 pandas DataFrame，并给它们添加列名，确保与训练时一致
    feature_names = ['data_size_gb', 'job_type_enc', 'executorMemoryMB', 'executorCores', 'driverMemoryMB',
